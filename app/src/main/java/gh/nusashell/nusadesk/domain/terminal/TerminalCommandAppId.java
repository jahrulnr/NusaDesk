...
/**
 * Stable identity of one user-defined terminal-command app.
 *
 * <p>The id is generated once by the host when the app is added and is never
 * derived from the display name or the command, so editing either cannot break
 * its stored record or a launcher tile that already points at it.</p>
 *
 ...
 */
public final class TerminalCommandAppId {
    ...
    private TerminalCommandAppId(String value) {
        this.value = value;
    }
     ...
     *                                  a storage key
     */
    public static TerminalCommandAppId of(String raw) {
        if (raw == null) {
            throw new IllegalArgumentException("terminalCommandAppId must not be null");
        }
        String trimmed = raw.trim();
        if (trimmed.isEmpty()) {
            throw new IllegalArgumentException("terminalCommandAppId must not be blank");
        }
        if (trimmed.length() > MAX_LENGTH) {
            throw new IllegalArgumentException(
                    "terminalCommandAppId must be at most " + MAX_LENGTH + " characters");
        }
        if (!ALLOWED.matcher(trimmed).matches()) {
            throw new IllegalArgumentException(
                    "terminalCommandAppId may only contain letters, digits, '-' and '_': "
                            + trimmed);
        }
        return new TerminalCommandAppId(trimmed);
    }
    ...
    public boolean equals(Object other) {
        ...
        if (!(other instanceof TerminalCommandAppId)) {
            return false;
        }
        return value.equals(((TerminalCommandAppId) other).value);
    }
